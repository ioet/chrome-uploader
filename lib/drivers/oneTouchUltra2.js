  ...
  return {
    ...
    connect: function (progress, data, cb) {
      ...
      cfg.deviceComms.connect(data.deviceInfo, handlers, probe, function(err) {
        if (err) {
          throw(new connectError('Error connecting to device'))
          return cb(err);
        }
    ...
    fetchData: function (progress, data, callback) {
      ...
      otu2CommandResponse(cmd, function(err, result){
          if (err) {
              throw(new fetchDataError('Failure trying to talk to device'));
    ...
    processData: function (progress, data, cb) {
      ...
      if (!data.post_records){
        throw(new processDataError('Error building upload post_records'))
      }
    ...
    uploadData: function (progress, data, cb) {
      ...
      if (!data.serialNumber || !data.id){
        throw(new uploadError('Missing device data'));
      }
    ...
    disconnect: function (progress, data, cb) {
      ...
      if (!data.disconnect){
        throw(new disconnectError('Device disconnect error'))
      }
      cb(null, data);
    },
